...
<div style="width: 100%; overflow-x: auto;">
  <span style="width: 44%; overflow-x: auto; display: inline-block; margin: auto;">
  ...
  <span style="width: 55%; overflow-x: auto; display: inline-block; margin: auto;">
...
to jointly measure the robustness and the accuracy of a model. The astuteness
of a model $h$ at radius $r > 0$ under a distribution $\mu$
...
As a side benefit, we can use this result to prove that the famous algorithm
[ID3](https://link.springer.com/content/pdf/10.1007/BF00116251.pdf)  for
learning decision trees returns a high-accuracy decision tree result, under
the linearly assumption. This is the __first__ time that a distributional
...
[many](https://arxiv.org/pdf/1911.07375.pdf)
...
scores, which were introduced at the beginning of the post.
In the decision tree that corresponds to the risk score, the right and left
subtrees are the same.

In the previous section we showed that decision stumps are a weak learner under
the linear separability assumption.
So why not use a boosting method to learn the entire interpretable risk score?
Specifically, we suggest to use the
[boost-by-majority](https://link.springer.com/content/pdf/10.1023/A:1010852229904.pdf)
boosting method in order to ensure that the weights are small integers. This
will lead to interptable model with size only $O(\gamma^{-2}\log1/\epsilon)$
where the model has accuracy $1-\epsilon.$
...
defiantly correct for the point without the noise.
...
model (this corresponds to the length of the risk score).
...
performs the best among the competitors \mm{what about the breastcancer
dataset?}.
When measured with the depth of the tree, BBM-RS performs competitively
with others.
...
accuracy, robustness, and interpretability. %and investigate how these
properties are connected.
We analyzed tree-based models and designed
an <ins>efficient</ins> tree-based algorithm that provably achieves all
these properties.
Our research is an important step towards building trustworthy models that
provably achieve achieve many desired properties.
